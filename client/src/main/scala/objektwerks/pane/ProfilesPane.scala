...
final class ProfilesPane(context: Context, model: Model) extends VBox:
  ...
  def add(): Unit =
    ProfileDialog(context, Profile(accountId = model.objectAccount.get.id, name = "")).showAndWait() match
      case Some(profile: Profile) => model.add(profile){ tableView.selectionModel().select(profile.copy(id = model.selectedProfileId.value)) }
      case _ =>
  ...
  def update(): Unit =
    if tableView.selectionModel().getSelectedItem != null then
      ...
      ProfileDialog(context, profile).showAndWait() match
        case Some(profile: Profile) => model.update(selectedIndex, profile) {
