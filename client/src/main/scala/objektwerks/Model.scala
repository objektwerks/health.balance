...
final class Model(fetcher: Fetcher) extends LazyLogging:
  ...
  def setDrinkablesWeekCalories() =
    val week = LocalDate.now.minusDays(8).getDayOfYear
    drinkablesWeekCalories.value = observableDrinkables.filter(d => Entity.epochSecondToDayOfYear(d.drank) > week).map(_.calories).sum.toString
  ...
  def setExpendablesWeekCalories() =
    val week = LocalDate.now.minusDays(8).getDayOfYear
    expendablesWeekCalories.value = observableExpendables.filter(e => Entity.epochSecondToDayOfYear(e.start) > week).map(_.calories).sum.toString
