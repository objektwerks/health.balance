...
final class Model(fetcher: Fetcher) extends LazyLogging:
  ...
  def profiles(): Unit =
    supervised:
      assertNotInFxThread("list profiles")
      fetcher.fetch(
        ListProfiles(objectAccount.get.license, objectAccount.get.id),
        (event: Event) => event match
          case fault @ Fault(_, _) => onFetchFault("profiles", fault)
  ...
  def add(profile: Profile)(runLast: => Unit): Unit =
    supervised:
      assertNotInFxThread(s"add profile: $profile")
      fetcher.fetch(
        AddProfile(objectAccount.get.license, profile),
        (event: Event) => event match
          case fault @ Fault(_, _) => onFetchFault("add profile", profile, fault)
          case ProfileAdded(id) =>
            observableProfiles.insert(0, profile.copy(id = id))
  ...
  def update(selectedIndex: Int, profile: Profile)(runLast: => Unit): Unit =
    supervised:
      assertNotInFxThread(s"update profile from: $selectedIndex to: $profile")
      fetcher.fetch(
        UpdateProfile(objectAccount.get.license, profile),
        (event: Event) => event match
  ...
  def update(selectedIndex: Int, drinkable: Drinkable)(runLast: => Unit): Unit =
    supervised:
      assertNotInFxThread(s"update drinkable from: $selectedIndex to: $drinkable")
      fetcher.fetch(
        UpdateDrinkable(objectAccount.get.license, drinkable),
        (event: Event) => event match
          case fault @ Fault(_, _) => onFetchFault("update drinkable", drinkable, fault)
  ...
  def update(selectedIndex: Int, expendable: Expendable)(runLast: => Unit): Unit =
    supervised:
      ...
      fetcher.fetch(
        UpdateExpendable(objectAccount.get.license, expendable),
        (event: Event) => event match
          case fault @ Fault(_, _) => onFetchFault("update expendable", expendable, fault)
