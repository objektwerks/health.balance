...
final class Dispatcher(store: Store, emailer: Emailer):
  ...
  private def addEdible(edible: Edible): Event =
    try
      EdibleAdded(
        supervised:
          retry( Schedule.fixedInterval(100.millis).maxAttempts(1) )( store.addEdible(edible) )
      )
    catch
  ...
  private def updateEdible(edible: Edible): Event =
    try
      EdibleUpdated(
        supervised:
          retry( Schedule.fixedInterval(100.millis).maxAttempts(1) )( store.updateEdible(edible) )
      )
    catch
  ...
  private def addDrinkable(drinkable: Drinkable): Event =
    try
      DrinkableAdded(
        supervised:
          retry( Schedule.fixedInterval(100.millis).maxAttempts(1) )( store.addDrinkable(drinkable) )
      )
    catch
  ...
  private def updateExpendable(expendable: Expendable): Event =
    try
      ExpendableUpdated(
        supervised:
          retry( Schedule.fixedInterval(100.millis).maxAttempts(1) )( store.updateExpendable(expendable) )
      )
    catch
